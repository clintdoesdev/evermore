...
const MAX_TILT_MOUSE = 12;
const MAX_TILT_GYRO = 10;

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value));
}

// Safari on iOS gates deviceorientation behind an explicit user-gesture permission prompt; no
// other browser does. Not in lib.dom's DeviceOrientationEvent type, hence the cast.
 ...
 * a bank balance. Tilts toward the pointer on desktop, and toward the device's own tilt on phones
 * that support it (iOS needs an explicit tap first — see the "Enable tilt" button — everything
 * else attaches automatically). Every number and row is decorative (not real user data) — this
 * only ever renders on the marketing site.
 */
export function PhoneMockup() {
  ...
  // ---- tilt, springed for a smooth trailing feel — driven by the mouse pointer on desktop, or
  // the device's own tilt on a phone that supports it. Both feed the same rotateX/rotateY pair,
  // so whichever input is actually available just works. ----
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    rotateY.set(px * MAX_TILT_MOUSE);
    rotateX.set(-py * MAX_TILT_MOUSE);
  }
  ...
  // ---- device-orientation tilt (phones only) ----
  const [needsTiltPermission, setNeedsTiltPermission] = useState(false);
  const [tiltButtonLabel, setTiltButtonLabel] = useState("Enable tilt");
  const baseOrientation = useRef<{ beta: number; gamma: number } | null>(null);

  // Calibrated to wherever the phone happened to be resting when it first started listening
  // (not absolute pitch/roll), same as a level-reset button — otherwise a phone that doesn't
  // naturally sit flat would leave the mockup permanently skewed.
  const handleOrientation = useCallback((event: DeviceOrientationEvent) => {
    if (event.beta === null || event.gamma === null) return;
    if (!baseOrientation.current) {
      baseOrientation.current = { beta: event.beta, gamma: event.gamma };
    }
    const deltaBeta = clamp(event.beta - baseOrientation.current.beta, -30, 30);
    const deltaGamma = clamp(event.gamma - baseOrientation.current.gamma, -30, 30);
    rotateX.set(-(deltaBeta / 30) * MAX_TILT_GYRO);
    rotateY.set((deltaGamma / 30) * MAX_TILT_GYRO);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- rotateX/rotateY are stable motion-value instances, safe to omit
  }, []);

  useEffect(() => {
    if (reduceMotion || typeof window === "undefined" || !("DeviceOrientationEvent" in window)) return;
    const DeviceOrientationEventIOS = window.DeviceOrientationEvent as IosDeviceOrientationEvent;
    if (typeof DeviceOrientationEventIOS.requestPermission === "function") {
      // iOS Safari: only offer it, never auto-attach — deviceorientation stays silent until the
      // visitor taps the button below (a required user gesture). This capability check can only
      // run client-side (no DeviceOrientationEvent during SSR), so the button's presence
      // necessarily lags one paint behind the initial (button-less) render rather than being
      // knowable up front — the standard, unavoidable shape for a client-only feature check.
  ...
  }, [reduceMotion, handleOrientation]);

  // Covers the iOS path above, where the listener is attached later from the button's own click
  // handler rather than from an effect — this still needs to come off on unmount (or a
  // client-side route change would otherwise leave it dangling on `window`).
  useEffect(() => {
    return () => window.removeEventListener("deviceorientation", handleOrientation);
  }, [handleOrientation]);
  ...
  async function enableTilt() {
    const DeviceOrientationEventIOS = window.DeviceOrientationEvent as IosDeviceOrientationEvent;
    try {
      const result = await DeviceOrientationEventIOS.requestPermission?.();
      if (result === "granted") {
        window.addEventListener("deviceorientation", handleOrientation);
        setTiltButtonLabel("Tilt enabled");
        setTimeout(() => setNeedsTiltPermission(false), 900);
      } else {
        setTiltButtonLabel("Tilt unavailable");
      }
    } catch {
      setTiltButtonLabel("Tilt unavailable");
    }
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {needsTiltPermission && !reduceMotion && (
        <button
          type="button"
          onClick={enableTilt}
          className="glass-card inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11px] font-semibold text-white/75 transition hover:text-white"
        >
